...
- Bot players: host adds AI bots; bots play autonomously with card-preference heuristics, counter a draw stack, declare LOCO!, call Contre-LOCO! and interject identical cards into an open window like any other player — except the window the deal opens on the round's first card, which is the humans' alone. Every reaction is delayed and probabilistic so they stay beatable.
...
- Identical-card interrupt (single + batch). The window has **no deadline**: it opens on the deal and on every card played, and closes only when somebody draws, passes, or the round ends. Anyone may take it, including the player who just played, the player whose turn it is, and a player dealt the twin of the card the round opens on — it is a race decided by arrival order at the server.
...
- **Contre-LOCO! is pressable from two cards out, not from the moment the server says so.** The button holds the centre of the action bar all match and goes live as soon as any opponent is down to 2 cards or fewer, i.e. one ordinary play before they can be caught, so the call is a read of the table rather than an answer to a cue. It stops exactly there: armed any earlier, it would be live through a stretch of the round where pressing it can only miss, and a miss you can schedule is a card you drew on purpose for a Swap to hand on. Hearing an opponent call LOCO! does not put it back to sleep, and that is on purpose: a button that went quiet would be telling you the call happened, and listening for it is your job. What lights up when somebody actually owes the call is the armed cue. A press that finds nobody on the hook costs 1 card — **once per card played**, however many times it is pressed, so a misread is a mistake and spamming it is not a second one. LOCO! is a small chip above the bar, on screen from the deal and lit only for the seconds you owe it.
...
- Round summary overlay with placements, points earned, cumulative scoreboard; auto-dismiss after 8 s or via Continue. It is an overlay and nothing more: the next round is dealt behind it and the board underneath stays live, so dismissing it never rewinds the table for whoever took their time reading.
