...
**A deploy never waits on the tables that are up.** The drain is 90 s and the grace period 150 s in both environments, so the shutdown is bounded by a constant rather than by how long a match runs, and `deploy_dev`, `deploy_prod` and `stop_dev` all carry `timeout: 10m` well clear of it. Production waited 15 minutes for a best-of-7 until 2026-08-02: it held a runner slot nobody else could use, and it left the pipeline one raised value away from failing on a match instead of on a fault. The snapshot is what pays for the shorter wait, and it makes the reconnect path the normal one in production rather than the exceptional one.
