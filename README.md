...
| `VITE_PUBLIC_ORIGIN` | *(production host)* | **Build-time.** The origin the canonical, `hreflang` and `og:` tags are made absolute against — crawlers do not run JS and never fetch a relative `og:image`, so it cannot be filled in at runtime. `client/Dockerfile` takes it as an `ARG`; `.gitlab-ci.yml` passes `https://${APP_HOST}`. |
