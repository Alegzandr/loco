...
function seedGameOver(over: Partial<ReturnType<typeof useGameStore.getState>>) {
...
describe('a matchmade table nobody is left at', () => {
  it('goes back to the queue by default', () => {
    ...
    expect(useGameStore.getState().screen).toBe('searching')
  })
  ...
  it('stays put while the opponent is still there to agree with', () => {
    ...
    expect(useGameStore.getState().screen).toBe('gameover')
  })
