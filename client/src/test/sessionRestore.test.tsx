...
import { initSessionRestore, RESTORE_TIMEOUT_MS } from '../hooks/useSessionRestore'
...
// Whatever App handed useWebSocket as getReconnectMsg, so the test can fire the
...
vi.mock('../components/GameView', () => ({ GameView: () => <div data-testid="game" /> }))
...
describe('reload into a live match', () => {
  it('boots onto the reconnect screen instead of the lobby', () => {
    ...
    expect(useGameStore.getState().screen).toBe('restoring')
  ...
  it('a refusal drops the record and returns to the lobby with a reason', () => {
    ...
    expect(useGameStore.getState().errorMsg).toBe('invalid session token for reconnect')
  ...
  it('restores a waiting room with a plain join', () => {
    ...
    expect(useGameStore.getState().restoreTarget).toBeNull()
  })
})
...
describe('what gets persisted', () => {
  ...
  it('clears the record when the match ends', () => {
    ...
    act(() => { useGameStore.getState().applyMatchEnd('Alice', []) })
    expect(readSession()).toBeNull()
  })

  it('never writes a record from the lobby', () => {
    renderApp()
    act(() => { useGameStore.getState().setError('room not found') })
    expect(readSession()).toBeNull()
  })
})
