...
function same(a: SafeAreaInsets, b: SafeAreaInsets): boolean {
  return a.top === b.top && a.right === b.right && a.bottom === b.bottom && a.left === b.left
}

/**
 * Current safe-area insets, re-measured when the device is rotated or the
 * window resized (landscape moves the notch from the top to one side).
 *
 * Deliberately not measured per frame: this is a device property, it changes on
 * rotation and nowhere else, and each read forces a style resolution. See
