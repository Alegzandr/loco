/**
 * How long to keep waiting on the room's render (the engine's chunk, the
 * models and the build) before telling the server we are ready anyway.
 *
...
export interface MapPreloadState {
  /** 0–1 across the room's load and render. Drives the bar, never a rules decision. */
  progress: number
  /** True once the render has settled — drawn, failed or timed out — and the full bar has been shown. */
  done: boolean
}
