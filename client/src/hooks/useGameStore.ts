 ...
 *
 * The state is a single object because that is what it is: the client's mirror
 * of one match. Several actions write across the families on purpose, since an
 * authoritative snapshot settles the board, the declarations and the scoreboard
 * in the same breath. What is split here is the *reading*, not the state.
 *
 * Nothing in here is authoritative. Every field is either the server's last
 * word or a presentation detail derived from it, and anything a rule depends on
 * is refused server-side whatever this store says.
 *
